...
<?php if (!empty($_SERVER['SCRIPT_FILENAME']) && 'comments.php' == basename($_SERVER['SCRIPT_FILENAME'])) die ('Please do not load this page directly. Thanks!'); ?>
	...
	<?php if (post_password_required()) :
		echo '<div class="password-notice">';
		echo _e('This post is password protected. Enter the password to view comments.');
		echo '</div>';
		return;
	endif; ?>

	<aside class="comments comments__wrapper">

	<?php if (have_comments()) : ?>

		<h3 class="comments__title"><?php comments_number('No Responses', 'One Response', '% Responses'); ?></h3>
		...
		<div class="comments__navigation">
			<span class="comments-navigation--next-posts"><?php previous_comments_link() ?></span>
			<span class="comments-navigation--prev-posts"><?php next_comments_link() ?></span>
		</div>
	...
	<?php else : /* this is displayed if there are no comments so far  */?>
		...
		<div class="comments-closed-message"><?php echo _e('Comments are closed.'); ?></div>
